...
![Логическая схема](https://i.ibb.co/k4Frms5/Screenshot-from-2022-12-07-00-35-27.png)
...
**Users**
```
4 + 128 + 128 + 128 + 128 + 2048 + 8 = 2572 байт
140 000 000 * 2572 = 360 080 000 000 байт = 0,36008 Тб
```
...
```
3*4 + 128 + 128 + 2048 + 8 = 2324 байт
# Если человек в среднем имеет 15 подписчиков, то 
2324 * 110 000 = 241 560 000 байт = 255,64 Мб
```
...
**Итог PostgresQL**
```
0,36008 Тб + 0,0252 Тб + 255,64 Мб ~= 0.4 Тб
```
...
![Схема проекта](https://i.ibb.co/kSRj2TS/Untitled-Diagram-drawio-3.png)

**Внешняя балансировка** - DNS балансировка.
**Внутренняя балансировка** - L7 на NGINX.
**Веб сервис** состоит из 3-ёх сервисов:
* **Стрим сервис** - сервис для работы с данными о стримах, здесь добавляются и раздаются ссылки на [HTTP Live Streaming](https://flussonic.ru/glossary/hls/) потоки
* **Сервис авторизации** - сервис работающий с данными пользователя и обрабатывающий запросы на авторизацию
* **Сервис зрительский и чата** - подлежит масштабированнию, высокий RPS 

Взаимодействие всех сервисов происходит благодаря gRPC, сами микросервисы написаны на go. 
Записи в базу данных происходят реже, чем чтение из PostgreSQL, но стримминговый сервис не требует точности как банковская система => Используем асинхронную репликацию. Данные будут записываться в мастер, а слейв базы данных будут обращаться к мастеру в read-only режиме и записывать новые изменения. Для того, чтобы данные не были утеряны, рядом с мастером будет расположен redis. Он будет хранить изменения в мастере за последние 10 минут. Это поможет держать актуальную информацию и при смерти мастера, получится легко заменить его слейвом, дополнив актуальной информацией из redis. 
Клиент и статические файлы раздаются NGINX, который проксирует запросы на сервисы.

![Схема общения по миру](https://i.ibb.co/Qj3J2CK/24.png)

**Сценарий стримера**
Когда стример создаёт стрим, он получает ссылку со Stream-KEY для ведения трансляции через OBS Studio или его аналоги, отправляющие RTMP поток на Ingest-сервер, который проверяет ключ стрима и перенаправляет его RTMP-поток в существующий AWS MediaLive транскодер или создает новый при отсутсвии свободных. 
После чего сгенерированные HLS-потоки отправляются на Edge сервер, далее по цепочке Edge сервера при необходимости забирают и ретранслируют поток ближе к зрителю.

**Сценарий зрителя**
Зритель сначала обращается к балансировщику, который по IP-адресу опредеяет его местоположение и направляет на оптимальный Edge сервер.

## 7. Список серверов
Пиковая нагрузка на просмотр стримов примерно 6000 гбит/сек. Если использовать 40 гбитные сетевые карты, то требуется
```
6000 * 6 / 40 = 900 штук
Возьмём с запасом: 1500 штук
```
Максимально активными одновременно может быть 90 000 стримов, edge сервер должен храненить последние 20 секунд стрима для последующей передачи стрима в 6 разных качествах со средним битрейтом 4500 кбит/с
```
4500 * 6 = 27 Мб
27 * 20 = 540 Мб
540 * 90 000 = 48,6 Тб
48,6 / 900 = 54 Гб ОЗУ
```
Значит каждый Edge сервер должен обладать минимум 54 гб ОЗУ, зная стандарты размеров оперативной памяти удобно использовать 16 x 4 = 64 Гб ОЗУ
Объем диска в Edge серверах можно взять SSD на 64 Гб, т.к. он не имеет осбого значения, основаня нагрузка приходится на оперативную память.

Разделение 1500 Edge серверов по местоположению в зависимости от регионов:
* Западное США - 15% Edge серверов = 225
* Восточное США - 15% Edge серверов = 225
* Германия - 10% Edge серверов = 150
* Корея - 10% Edge серверов = 150
* Россия - 10% Edge серверов = 150
* Франция - 10% Edge серверов = 150
* Остальные 30% = 450 Edge серевров распределены по всему миру 

**Edge-сервер. Конфигурация**
CPU | RAM (Гб) | Тип диска | Объем диска (Гб) | Пропускная способность (Гб/с)
--- | -------- | --------- | ---------------- | -----------------------------
6   | 64       | SSD       |   64             | 40  

Входящий трафик в пиковой нагрузке равен примерно 2500 Гб/сек (Используются те же сетевые карты)
```
2500 * 6 / 40 = 375
```
На Ingest сервер приходит один поток максимально высокого качества RTMP стрима.

**Ingest-сервер. Конфигурация**
CPU | RAM (Гб) | Тип диска | Объем диска (Гб) | Пропускная способность (Гб/с)
--- | -------- | --------- | ---------------- | -----------------------------
4   | 16       | SSD       |   64             | 40  

**Nginx-сервер. Конфигурация**
CPU | RAM (Гб) | Тип диска | Объем диска (Гб) | Пропускная способность (Гб/с)
--- | -------- | --------- | ---------------- | -----------------------------
16   | 32       | SSD      |   128             | 20

**Backend-сервер. Конфигурация**
CPU | RAM (Гб) | Тип диска | Объем диска (Гб) | Пропускная способность (Гб/с)
--- | -------- | --------- | ---------------- | -----------------------------
64  | 24       | SSD       |   60             | 20  

**Бд-сервер. Конфигурация**
CPU | RAM (Гб) | Тип диска | Объем диска (Гб) | Пропускная способность (Гб/с)
--- | -------- | --------- | ---------------- | -----------------------------
16  | 12       | SSD, HDD  |   256, 1000      | 20  

**Распределение датацентров:**
* Западное США - 3 датацентра
* Восточное США - 3 датацентра
* Германия - 2 датацентра
* Корея - 2 датацентра
* Россия - 2 датацентра
* Франция - 2 датацентра
* Остальные 30% - 6 датацентра 

**ИТОГО** - 20 датацентров

**Количество серверов на один датацентр**
...
12. https://flussonic.ru/glossary/hls/
